...
const UnitListItem: React.FC<UnitListItemProps> = ({ unit, isSelected, onClick, onMouseEnter }) => {
  ...
  const selectionClasses = isSelected ? 'selected-list-item-pulse' : 'hover:bg-gray-800/60';
  const selectedStyle = isSelected ? { boxShadow: `0 0 0 2px rgba(var(--selection-glow-rgb), 0.5)` } : {};
  ...
  return (
    <li
      ...
      style={selectedStyle}
      className={`p-2.5 rounded-lg bg-black/40 border-l-4 ${borderColor} cursor-pointer ${selectionClasses} transition-all duration-200 transform hover:-translate-y-px`}
    >
      <div className="flex justify-between items-center mb-1.5">
        ...
        <p className="text-sm text-gray-400 font-mono">ATK: {attack}</p>
      </div>
      <div className="w-full bg-black/30 rounded-full h-2 border border-black/20 overflow-hidden">
        <div
          className={`h-full rounded-full bg-green-500 transition-all duration-500 ease-out ${isSelected ? 'selected-health-bar' : ''}`}
