...
const Tile: React.FC<TileProps> = ({ tile, isSelected, onSelect, gameState }) => {
  ...
  // Since we removed multi-tile logic, these are simpler
        ...
        >
          ...
          {infrastructureAsset && (
            <img
              ...
              className="absolute transition-transform group-hover:scale-105 object-contain"
              style={{
                // Larger assets for settlements, smaller for other infra
                width: isSettlement ? '150%' : '60%',
                height: isSettlement ? '150%' : '60%',
                left: '50%',
                top: '50%',
                // Position settlements from their bottom center, others from their visual center
                transform: isSettlement ? 'translate(-50%, -85%)' : 'translate(-50%, -70%)',
              }}
              draggable="false"
            />
          )}
          ...
          {worldEventAsset && (
            <img src={worldEventAsset} alt={worldEvent?.name} className="absolute object-contain transition-transform group-hover:scale-110" style={{ width: '60%', height: '60%', left: '50%', top: '50%', transform: 'translate(-50%, -70%)' }} draggable="false" />
          )}
          ...
          {unitAsset && unitFaction && (
            <div className="absolute" style={{ width: '60%', height: '60%', left: '50%', bottom: '40%', transform: 'translateX(-50%)' }}>
              ...
              <img 
                src={unitAsset} 
                alt={unitDef?.name} 
                className={`absolute w-full h-full object-contain transition-transform group-hover:scale-110 ${isUnitSelected ? 'unit-selection-glow' : ''}`} 
                draggable="false" 
              />
               {showAttackFlash && (
