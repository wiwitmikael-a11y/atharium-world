...
const formatMintedAthar = (num: number) => new Intl.NumberFormat('en-US', { notation: 'compact', maximumFractionDigits: 1 }).format(num);
...
const Header: React.FC<HeaderProps> = (props) => {
  ...
  return (
    <>
      <header className="absolute top-0 left-0 right-0 bg-gray-900/90 p-2 shadow-lg z-20 backdrop-blur-lg border-b-2 border-cyan-400 shadow-cyan-400/30 safe-area-inset-top">
        <div className="container mx-auto flex justify-between items-center text-white">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
                ...
                <button onClick={() => { setIsSettingsOpen(p => !p); soundManager.playSFX('ui_click_subtle'); }} className="p-2 bg-gray-800 border border-gray-700 rounded-full hover:bg-gray-700 transition-colors" aria-label="Settings"><Icon name="crystal" className="w-5 h-5 md:w-6 md:h-6 text-cyan-400" /></button>
                ...
                <button onClick={() => { onToggleHelp(); soundManager.playSFX('ui_click_subtle'); }} className="p-2 bg-gray-800 border border-gray-700 rounded-full hover:bg-gray-700 transition-colors hidden sm:block" aria-label="Help"><Icon name="help" className="w-5 h-5 md:w-6 md:h-6 text-cyan-400" /></button>
            </div>

            {/* Time Display */}
            <div className="flex items-center space-x-2 bg-gray-800 px-3 py-1.5 rounded-lg border border-gray-700">
              <div onClick={(e) => setActiveTooltip(activeTooltip?.id === 'time' ? null : { id: 'time', element: e.currentTarget as HTMLElement })} className="flex items-center space-x-1 sm:space-x-2 cursor-pointer" title="Game Date">
                <Icon name="time" className="w-4 h-4 text-cyan-400" />
                <span className="font-mono text-xs sm:text-base whitespace-nowrap">{currentMonthName} {yearNumber}</span>
              </div>
              <div className="w-px h-4 bg-gray-600 hidden md:block"></div>
              <div onClick={(e) => setActiveTooltip(activeTooltip?.id === 'epoch' ? null : { id: 'epoch', element: e.currentTarget as HTMLElement })} className="hidden md:flex items-center space-x-2 cursor-pointer" title="Current Epoch">
                <Icon name="epoch" className="w-4 h-4 text-purple-400" /><span className="text-sm sm:text-base truncate max-w-[100px] lg:max-w-none">{epochInfo?.name || '...'}</span>
              </div>
            </div>

            {/* Resources (Desktop only for full view, simplified for mobile if needed) */}
            <div className="hidden lg:flex items-center space-x-2">
              <div onClick={(e) => setActiveTooltip(activeTooltip?.id === 'athar' ? null : { id: 'athar', element: e.currentTarget as HTMLElement })} className="flex items-center space-x-2 w-48 cursor-pointer" title="$ATHAR Minted">
                <span className="text-xs font-semibold text-red-400">$ATHAR</span>
                <div className="flex-1 bg-gray-700 rounded-full h-2">
                    <div className="bg-red-500 h-2 rounded-full transition-all duration-500" style={{ width: `${atharProgress}%` }}></div>
                </div>
                <span className="font-mono text-sm font-bold">{formatMintedAthar(totalMintedAthar)}</span>
              </div>
              
              <div className="relative">
                  <button
                      onClick={() => { setIsFactionsListOpen(prev => !prev); soundManager.playSFX('ui_click_subtle'); }}
                  ...
                  {isFactionsListOpen && (
                      <FactionsListDialog
                          ...
                          onSelectFaction={(factionId) => { onSelectFaction(factionId); setIsFactionsListOpen(false); }}
          ...
          <div className="flex items-center">
            <div className="flex items-center bg-gray-800 border border-gray-700 rounded-full p-1 gap-1">
              {SPEEDS.map(speed => (
                <button key={speed} onClick={() => onSetSpeed(speed)} onMouseEnter={() => soundManager?.playUIHoverSFX()} className={`w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full transition-colors duration-200 ${gameSpeed === speed ? 'bg-cyan-600 text-white shadow-md' : 'text-gray-300 hover:bg-gray-700'}`} title={speed === 0 ? 'Pause' : `Speed ${speed}x`}>
                  {speed === 0 ? <Icon name="pause" className="w-3 h-3 sm:w-4 sm:h-4" /> : <span className="text-xs sm:text-sm font-bold">{speed}x</span>}
                </button>
              ))}
            </div>
          </div>
